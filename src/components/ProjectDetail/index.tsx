    ...
    <main>
      ...
      <section className="relative overflow-hidden bg-[#0F2018]">
        <figure className="relative w-full aspect-[16/8] min-h-[480px] overflow-hidden">
          <img src={(typeof project.heroImage === 'object' && project.heroImage !== null ? ((project.heroImage as any).default?.src || (project.heroImage as any).src || (project.heroImage as any).default || project.heroImage) : project.heroImage)} alt={project.title} className="h-full w-full object-cover opacity-70" />
          ...
          <div className="absolute inset-0 flex flex-col justify-end p-8 sm:p-16 pt-44 sm:pt-48 text-linen">
            <Link href="/portfolio" className="inline-flex items-center gap-2 text-sm text-linen/70 hover:text-linen mb-8 transition-colors w-fit">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-8 grid grid-cols-1 lg:grid-cols-12 gap-10">
          <div className="col-span-1 lg:col-span-4">
            <div className="sticky top-32 space-y-8">
              ...
              <dl className="grid gap-5 border-t border-forest/10 pt-6">
                {[
                  ...
                  { label: 'Duration', value: project.duration },
                  {
                    ...
                    href:
                      project.category === 'Veterinary'
                        ? '/services/veterinary-hospital-construction-nj'
                        : project.category === 'Commercial'
                        ? '/services/boutique-office-construction'
                        : '/services/whole-home-additions',
                  },
                ].map(({ label, value, href }) => (
                  <div key={label}>
                    <dt className="text-[10px] uppercase tracking-[0.22em] text-olive">{label}</dt>
                    <dd className="mt-1 text-sm text-forest font-medium">
                      {href ? (
                        <Link href={href} className="text-forest hover:text-terracotta underline decoration-forest/30 underline-offset-4 transition-colors">
                          {value} →
                        </Link>
                      ) : (
                        value
              ...
              <Link href="/contact" className="inline-flex items-center gap-2 rounded-full bg-forest px-6 py-3 text-sm font-medium text-linen transition-transform hover:scale-[1.02]">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              {project.galleryImages.map((img, i) => (
                <figure key={i} className={`overflow-hidden rounded-[20px] ${i === 0 ? 'sm:col-span-2 aspect-[16/9]' : 'aspect-[4/3]'}`}>
                  <img src={(typeof img === 'object' && img !== null ? ((img as any).default?.src || (img as any).src || (img as any).default || img) : img)} alt={`${project.title} — view ${i + 1}`} className="h-full w-full object-cover transition-transform duration-700 hover:scale-105" loading="lazy" />
                </figure>
              ))}
            </div>
          </div>
      ...
      <section className="bg-sand py-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-8">
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            {others.map((p) => (
              <Link key={p.id} href={`/portfolio/${p.id}`} className="group relative overflow-hidden rounded-[24px]">
                <figure className="aspect-[4/3] overflow-hidden">
                  <img src={(typeof p.heroImage === 'object' && p.heroImage !== null ? ((p.heroImage as any).default?.src || (p.heroImage as any).src || (p.heroImage as any).default || p.heroImage) : p.heroImage)} alt={p.title} className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-[1.06]" loading="lazy" />
          ...
          <div className="mt-10 text-center">
            <Link href="/portfolio" className="inline-flex items-center gap-2 rounded-full border border-forest px-7 py-3 text-sm font-medium text-forest transition-all hover:bg-forest hover:text-linen">
              View all projects →
            </Link>
          </div>
        </div>
